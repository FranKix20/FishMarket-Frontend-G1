...
export default function ChatWidget() {
  ...
  const [messages, setMessages] = useState([
    {
      from: 'bot',
      text: '¡Hola! 👋 Soy el asistente de FishMarket Cloud. Pregúntame por envíos, pagos o productos. Recuerda iniciar sesión para consultar tus pedidos.',
      isWelcome: true
    }
  ]);
      ...
      if (prev.length === 1 && prev[0].isWelcome) {
        return [{
          ...
          text: user
            ...
            : '¡Hola! 👋 Soy el asistente de FishMarket Cloud. Pregúntame por envíos, pagos o productos. Recuerda iniciar sesión para consultar tus pedidos.',
          isWelcome: true
        }];
      }
      return prev;
    ...
    try {
      ...
      if (err.status === 401 || err.code === 'UNAUTHORIZED') {
        setMessages((m) => [
          ...
          { 
            from: 'bot', 
            text: '🔒 **Acceso Protegido**\n\nNo has iniciado sesión o tu sesión ha expirado. Por favor, ingresa a tu cuenta para que pueda ayudarte con la información de tus pedidos.',
            authRequired: true 
          }
        ]);
      } else {
                        ...
                        {m.authRequired && (
                          <div className="chat-widget__auth-card">
                            ...
                            <div className="chat-widget__auth-card-actions">
                              ...
                              <button 
                                type="button" 
                                className="chat-widget__auth-btn chat-widget__auth-btn--secondary"
                                onClick={() => {
                                  setOpen(false);
                                  navigate('/registro');
                                }}
